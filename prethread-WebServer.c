...
void handle_request(int client_fd) {
    // Límite de peticiones: si se supera, enviamos 503 y cerramos conexión, sin detener el servidor
...
void parse_args(int argc, char *argv[], server_config_t *cfg) {
    ...
    while ((opt = getopt(argc, argv, "n:w:p:T:L:")) != -1) {
        switch (opt) {
            ...
            default:
                fprintf(stderr, "Uso: %s -n <hilos> -w <root> -p <puerto> [-T <protocolo>] [-L <límite>]\n", argv[0]);
                exit(EXIT_FAILURE);
        }
    }
